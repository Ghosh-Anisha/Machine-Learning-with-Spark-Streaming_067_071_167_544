...
def preprocessing(df):
	...
	X_train_counts = count_vect.fit_transform(df_n).toarray()
	
	tfidfconverter = TfidfTransformer()
	tf_transformer= tfidfconverter.fit_transform(X_train_counts).toarray()
	
	tfidfconverter = TfidfVectorizer(max_features=1500, min_df=5, max_df=0.7, stop_words=stopwords.words('english'))
	X = tfidfconverter.fit_transform(tf_transformer).toarray()
