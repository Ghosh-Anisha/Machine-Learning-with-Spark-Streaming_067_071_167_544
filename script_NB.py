...
def preprocessing(df):
	...
	for sen in range(0, len(df_temp)):
	    ...
	    #df_temp1 = [word_tokenize(word) for word in df_temp1]
	    #df_temp1 = [ps.stem(word) for word in df_temp1]
	...
	X_train = func.fit_transform(X_train)
	df3=clf1.partial_fit(X_train,y_train,classes=[0,4])
	#pickle.dump(df2,open('model.pkl','wb'))
	pickle.dump(df3,open('model1.pkl','wb'))
	y_pred=df3.predict(X_test)
